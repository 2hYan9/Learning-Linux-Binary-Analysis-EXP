...
代码混淆技术（也称为代码转换技术）是通过修改汇编层的代码来引入不明确的分支指令或未对齐的指令，使得反汇编程序无法正确地读取二进制（机器码）文件。
...
可以看到调用 antidebug 本来要执行的是 `mov $0x31337, %%eax` 指令，但是我们在这条指令之前插入一个字节 0xe9，这是跳转指令的前缀，于是反汇编器 objdump 错误将 antidebug 的第一条指令解释为跳转指令，跳转到一个意义不明的地址。但是我们调用 antidebug 的时候都是跳过这个字节的，所以能够程序能够正常执行。

代码转换并不会改变代码的功能，而是转换代码的形式。不过上面的代码转换方法并不能骗过 IDA 这样的智能反编译器，因为 IDA 生成反汇编代码的时候使用的是控制流分析。
