...
**对于系统调用的参数，寄存器的使用顺序则会有点不同**，每个参数从左到右的顺序为：
...
> *对注入的代码进行调试*：
> 因为在注入代码时，宿主程序会被追踪者程序所追踪，而一个程序同时只能被一个程序追踪，因此在宿主程序执行的过程中无法使用strace或者gdb这样的工具来调试宿主程序，所以很难知道注入到宿主程序中的代码是如何执行的。
> 然而在进行测试的时候，我们是可以修改用于测试的宿主程序的源码的，因此，我们可以直接将准备注入到宿主程序的代码写到宿主程序的源码中，编译后使用strace或者gdb对宿主程序进行调试，这样就能够知道注入的代码是如何执行的。
...
首先需要通过特定感染机制(infection machanism)选择可供感染的目标，感染目标可以是二进制文件，也可以是正在运行中的进程。然后通过某种方法感染这个目标，也就是向这个感染目标中添加该目标本不该执行的代码，这样的代码通常被称为寄生代码(parasited code)，被感染后的目标通常被称为宿主程序。

无论是二进制文件还是进程，它们都有一个控制流，也被成为执行路径。因此病毒在感染了宿主程序后，下一步就是劫持宿主程序控制流。劫持控制流的方法有很多，但通常都需要让宿主程序触发某个trigger后才能劫持到宿主程序的控制流。

函数蹦床是一种常见的控制流劫持方法，在宿主程序调用了某个被插入函数蹦床的函数后，宿主程序的控制流将会被插入其中的寄生代码所劫持，也就是宿主程序会转而执行原本不该执行的寄生代码，而寄生代码的作用通常是加载并执行病毒的有效载荷(payload)。

这一章节的内容中，将会先介绍常见的代码寄生技术，也就是在宿主程序的二进制文件中找到一块合理的区域来存放我们的代码；然后会介绍如何劫持控制流，让宿主程序能够执行我们寄生的代码；最后会介绍一些进程内存感染的方法，这和前面所介绍的代码注入有些类似，但是这里将会进行更加详细的介绍。
...
最后，为了能够让宿主程序执行寄生代码，这里使用的方法是直接将宿主程序的程序入口点修改为寄生代码插入的地址，即phdr[text].p_vaddr + phdr[text].p_memsz。这是一种简单粗暴的控制流劫持方法，虽然很容易实施，但是也容易被发现。

同时为了能够让宿主程序也正常执行（这样不容易被发现），寄生代码在执行完成后还需要跳转到宿主程序的原来的程序入口点，将控制移交给宿主程序。如下：
...
换句话说，逆向text段感染实际上是一个失败的（静态）二进制插桩实例。但是如果不考虑宿主程序能否正常执行，逆向text段感染也确实是一种比较隐蔽的感染方法。
...
![loadable_note](./image/loadable_note.png)
...
不过想要精确劫持某个函数的调用，实现起来会比较复杂，因为定位函数的地址需要依赖宿主程序的符号表和字符串表。不过除了定位欲劫持函数的偏移位置，其他的步骤基本和PLT直接感染一致，而在前面的A simple debugger编程实战中也演示了如何查找符号表的方法。
...
在前面介绍进程追踪那部分内容时介绍过一部分代码注入的内容，这其实就是一种进程感染的方式，而这里则是更加深入地讲解进程内存感染。

进程感染不需要对磁盘上的内容进行修改，因此比二进制文件感染要更加隐蔽。虽然进程内存感染的过程欲ELF二进制文件感染有所区别，但是前面所讨论的感染点也是可以在进行进程内存感染时使用的。

由于是在内存中，因此必须把寄生代码加载到内存中，可以通过PTRACE_POKETEXT直接注入，也可以通过注入shellcode创建一个新的内存映射来存储payload。
...
一种比较简单的方法是使用环境变量 LD_PRELOAD ，可以通过设置 LD_PRELOAD 环境变量，将我们想要的共享库放在其他共享库之前加载。这种方法其实算不上是程序执行过程中的代码注入，因为这种方法主要是影响程序执行之前动态链接器加载共享对象的过程。

使用命令 export 可以设置一个环境变量，使用命令 unset 可以删除一个环境变量，如下图，使用 LD_PRELOAD 环境变量让宿主程序在执行时加载了另一个共享目标。

![ld_preload](./image/ld_preload.png)

这样的方法十分方便，劫持glibc的成本很低，但是也很容易被发现。

##### dlopen()

在前面介绍代码注入的时候，我们采用的方法是向目标进程的代码填充区插入一段shellcode，这段代码会使用mmap()系统调用开辟一块匿名的地址空间，然后将控制返回到追踪者进程，追踪者进程会将作为payload可执行文件使用PTRACE_POKETEXT写入到这块匿名地址空间，然后将目标进程的PC设为payload的程序入口点以使目标进程执行payload。

但是这样的方法面临的一个问题是大多数要注入到进程中的共享库都需要进行重定位，使用mmap()或者open()只会将文件加载到内存，而不会处理代码重定位。因此，对于大多数想要加载到内存中的共享库，除非是完全地址独立的代码，否则很难正确执行。

解决这个问题的方法是使用dlopen()库函数，一个可执行文件在没有第一时间被链接的情况下，会使用dlopen()函数来进行运行时加载共享库。
...
函数 dlopen() 用以加载一个动态共享目标文件，这个文件由参数中的以 0 结尾的字符串 *filename* 指定，然后返回一个不透明的 "handle" 给调用者，也就是调用者无法知道 "handle" 中的具体成员，但是可以调用其他函数对 "handle" 进行解析。 

如果参数 *filename* 中包含反斜杠 "/" ，那么这个参数将会被解释为一个相对或者绝对地址，否则，动态链接器将会使用默认规则进行加载，前面已经介绍过这个默认规则。

此外，参数 *flag* 必须包含下面两个选项中的一个：

- RTLD_LAZY: 执行延迟绑定(lazy binding)，也就是只有到进程执行共享库中的函数时才进行解析。延迟绑定只用于函数引用，对于变量的引用都是立即绑定；
- RTLD_NOW: 如果设置了这个选项，或者环境变量 LD_BIND_NOW 不是空字符串，那么共享目标中所有未定义的符号都会在 dl_open() 返回之前完成解析，如果无法成功解析，那么 dl_oepn() 将会返回一个错误。
...
通常情况下，病毒和恶意代码会使用经过加密或者通过保护机制进行封装，主要的目的是为了反调试，避免被安全工作人员二进制分析。

### ELF反调试与封装技术

这类技术主要是病毒或者恶意代码防止被人在运行时追踪的手段的。

比如之前介绍过的使用 PTRACE_TRACEME 进行反调试：
...
病毒或者恶意代码调用上面这样的函数后，如果被调试器调试，那么就会终止自身的进程，否则，上面的函数会成功追踪自身，使得其他调试器无法追踪该进程。

但是正如前面所提到的，这种基于ptrace系统调用的反调试方法是可以绕过的，比如使用LD_PRELOAD环境变量，来诱骗程序加载一个假的ptrace系统调用。

#### SIGTRAP handler 技术

在调试的时候通常会设置断点，程序执行到断点处会产生一个SIGTRAP信号，调试器的signal handler会捕获到这个SIGTRAP信号，使被追踪的程序会暂停，并检查该程序的执行状态。

那么病毒或者恶意代码就可以自己故意执行一个断点指令，并且自行捕获这个断点指令产生的SIGTRAP信号，如果能够捕获到，那么就说明当前的程序没有被追踪，否则就说明程序已经被追踪。如下：
...
上面这样的代码可以检测当前进程是否被追踪。

#### /proc/(pid)/status 技术

可以发现，程序反调试的底层思路都是检查自己是否被追踪，除了前面所介绍的PTRACE_TRACEME技术和SIGTRAP handler技术，程序还可以通过检查一些动态文件来判断当前是否被别的程序追踪。

在文件 /proc/\<pid\>/status 中记录了由pid指定的进程的执行状态，如下：

![proc_self_status](./image/proc_self_status.png)

这是该文件的部分内容，注意文件中的tracer id字段，这个字段表示追踪当前程序的程序的pid，如果是 0 则表示当前进程没有被追踪。因此，程序可以通过读取该状态文件来检查自己是否被追踪，从而达到反调试的目的。

### 代码混淆

前面所介绍的二进制保护机制都是基于程序动态执行过程中的反调试技术，实际上，病毒或者恶意代码也可以通过静态的修改来避免被分析。

代码混淆技术（也称为代码转换技术）是通过修改汇编层的代码来引入不明确的分支指令或未对齐的指令，使得反汇编程序无法正确地读取二进制（机器码）文件。常见的代码混淆技术是基于指令重叠进行代码混淆。

在常规反汇编器的假设中，二进制文件中的每字节最多映射到一条指令，换句话说，反汇编器通常建设指令之间互相不重叠，而基于指令重叠的代码混淆则打破了这种假设。

出现指令重叠的原因是因为在X86平台上指令的长度是不相等，也就是说X86指令不全是由等长的指令组成的。因此，处理器不会在内存中强制进行指令地址对齐，从而使得某一条指令占用另一条指令的地址。
...
这样的代码编译完成后，可以看到objdump反汇编的代码，如下所示：
...
0000000000001000 <_start>:
    ...
    1008:	eb 01                	jmp    100b <antidebug+0x1>

000000000000100a <antidebug>:
    100a:	e9 00 b8 37 13       	jmp    1337c80f <__bss_start+0x1337880f>
    100f:	03 00                	add    (%rax),%eax
    1011:	90                   	nop
    1012:	5d                   	pop    %rbp
    1013:	c3                   	ret    
```

从源码可以看到调用 antidebug 本来要执行的是 `mov $0x31337, %%eax` 指令，但是我们在这条指令之前插入一个字节 0xe9，这是跳转指令的前缀，于是反汇编器 objdump 错误将 antidebug 的第一条指令解释为跳转指令，跳转到一个意义不明的地址。但是我们调用 antidebug 的时候都是跳过这个字节的，所以能够程序能够正常执行。

而从二进制的角度可以看到，位于偏移量0x1008的跳转指令所跳转的目的地址是位于偏移量0x100a的指令的中间，也就是跳转指令的目的地址的指令，位于0x100b偏移量的指令，与位于0x100a偏移量的指令发生了指令重叠。

代码转换并不会改变代码的功能，而是转换代码的形式。不过上面的代码转换方法并不能骗过 IDA 这样的智能反编译器，因为 IDA 生成反汇编代码的时候使用的是控制流分析。此外，也可以使用自定义的反汇编绕过这样的保护机制。

### 二进制加壳

加壳(pack)是指恶意软件作者对可执行文件进行压缩或者加密，来对代码和数据进行混淆。

一种常见的加壳器(packer)是UPX，这个工具在大多数Linux发行版中都作为包的形式存在，其初衷是将可执行文件压缩成更小的体积。

由于代码被压缩，因此程序在内存中执行之前需要通过某种方式进行解压。而由于可执行文件被压缩后代码进行了混淆，所以恶意软件的作者会对已经被感染的可执行文件进行压缩以躲过杀毒软件的检测，不过现在的大多数杀毒产品中都使用了自动脱壳器。

现在一般将使用了任意形式的模糊层进行屏蔽的二进制文件都称为加壳二进制文件，而不仅仅指压缩过的文件。

#### 存根机制

这里将会对一些常见的加壳器，或者说是软件保护器，进行一个简单的介绍。

一般来说，一个加壳器中包含两种代码：

1. 保护阶段的代码(protection phase code)： 这部分代码主要是对要保护的二进制文件进行加密和封装；
2. 存根(stub)： 这部分的代码主要是让封装过的二进制文件能够脱壳并正常执行，存根一般是和封装过后的二进制文件合并在一起的。再有些资料中也被称为运行时引擎或者引导代码。

下图是二进制文件保护的常用的方法：

![protected_binary](./image/protected_binary.png)

可以看到被保护的二进制文件被封装过后会存储到一个新的二进制文件的DATA段，并且这个新的二进制文件通常只有两个段，TEXT段和DATA段。

这里主要是对存根进行介绍。

在使用软件保护器的情况下，被保护的二进制文件是被加密了的，在执行之前需要进行解密。

早期的软件保护器通常是将被保护的二进制文件解密解封装后写入到磁盘中的另一个文件中，然后调用 execve() 执行这个文件。这种软件保护方法显然是有缺陷的。

所以现在都是采用另一种方法，也就是在内存中对被保护的二进制文件进行解密解封装，然后直接加载到内存中去执行。
...
也就是说，我们要在存根中使用用户层代码(userland code)实现一个execve()系统调用，在书中被称为用户层执行(userland exec)。

> 注意这里为什么要使用用户层代码实现execve()系统调用，因为我们并不想让解密解封装后的二进制文件以明文的形式写入到磁盘后再执行，而是直接在内存中解密解封装后就直接加载到指定的地址范围中执行。

存根文件负责对嵌入其中的可执行文件进行解密，然后将解密后的文件的PT_LOAD段加载到程序头所指定的地址中。

如果可执行文件的加载地址和存根的加载地址冲突了，被保护的程序就无法正常执行。这就意味着存根程序需要使用自定义的链接器脚本，这一般是通过修改由ld使用的现有链接器脚本来实现的。

想要分析已经加壳的二进制文件，首先需要对其进行脱壳处理，这个过程通常使用二进制插桩的方法进行。
