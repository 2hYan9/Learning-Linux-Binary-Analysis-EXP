...
首先最常见的工具就是GDB，主要是用于调试程序，当然也可以用来查看程序运行时的内部状态，比如代码、寄存器、以及内存，这里就不做过多介绍。
...
> 在这个代码寄生和前面的代码注入的实验中，注入或者寄生到宿主程序的shellcode都是在tracer程序中使用内联汇编的方式生成的，这可能是这两个实验都以类似的方式出现错误的一个原因。而在其他地方找到的用以实现代码注入的源代码中，都是以机器码的形式进行注入，后面可以尝试直接使用机器码进行代码注入和寄生。  
> 这种方式生成的shellcode可能会破坏宿主程序的栈帧结构从而引发段错误。
